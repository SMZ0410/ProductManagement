...
namespace DAL.RoleInfo
{
    public class RoleInfoDal : BaseDal<RoleInfoDal>
    {
        ...
        public List<RoleInfoModel> GetRoleInfos(string RoleName)
        {
            ...
            {
                string sql = "SELECT ri.RoleId,ri.RoleName,p.privilegeName,(SELECT COUNT(*) FROM dbo.UserInfo)AS UserNum FROM dbo.RoleInfo ri JOIN RolePrivilegeMapInfo rp ON ri.RoleId = rp.RoleId JOIN PrivilegeInfo p ON rp.PrivilegeId = p.PrivilegeId JOIN dbo.UserRoleMapInfo u ON ri.RoleId = u.RoleId JOIN dbo.UserInfo ui ON ui.UserId = u.RoleId WHERE ri.RoleName LIKE @rolename AND ri.Status = 1";
